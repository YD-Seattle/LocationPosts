...
if ( ! class_exists( 'YD_Cron' ) ) {
	 ...
	 */
	class YD_Cron extends YD_Module {
		...
		public static function add_custom_cron_intervals( $schedules ) {
			$schedules[ 'yd_debug' ] = array(
				'interval' => 5,
				'display'  => 'Every 5 seconds'
			);

			$schedules[ 'yd_ten_minutes' ] = array(
				'interval' => 60 * 10,
				'display'  => 'Every 10 minutes'
			);

			$schedules[ 'yd_example_interval' ] = array(
				'interval' => 60 * 60 * 5,
		...
		public static function fire_job_at_time() {
			...
			if ( (int) date( 'G', $now ) >= 1 && (int) date( 'G', $now ) <= 3 ) {
				if ( ! get_transient( 'yd_cron_example_timed_job' ) ) {
					//YD_CPT_Example::exampleTimedJob();
					set_transient( 'yd_cron_example_timed_job', true, 60 * 60 * 6 );
				}
			}
		}
		...
		public function register_hook_callbacks() {
			add_action( 'yd_cron_timed_jobs',  __CLASS__ . '::fire_job_at_time' );
			add_action( 'yd_cron_example_job', __CLASS__ . '::example_job' );
		...
		public function activate( $network_wide ) {
			if ( wp_next_scheduled( 'yd_cron_timed_jobs' ) === false ) {
				wp_schedule_event(
			...
			if ( wp_next_scheduled( 'yd_cron_example_job' ) === false ) {
				wp_schedule_event(
					current_time( 'timestamp' ),
					'yd_example_interval',
					'yd_cron_example_job'
		...
		public function deactivate() {
			wp_clear_scheduled_hook( 'yd_cron_timed_jobs' );
			wp_clear_scheduled_hook( 'yd_cron_example_job' );
		}
