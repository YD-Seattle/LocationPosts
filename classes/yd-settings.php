...
if ( ! class_exists( 'YD_Settings' ) ) {

	/**
	 * Handles plugin settings and user profile meta fields
	 */
	class YD_Settings extends YD_Module {
		protected $settings;
		 ...
		 * @param array  $value This will be merged with YD_Settings->settings, so it should mimic the structure of the YD_Settings::$default_settings. It only needs the contain the values that will change, though. See YD_Plugin_Skeleton->upgrade() for an example.
		 */
		public function __set( $variable, $value ) {
			// Note: YD_Module::__set() is automatically called before this
			...
			update_option( 'yd_settings', $this->settings );
		}
		...
		public static function markup_settings_page() {
			if ( current_user_can( self::REQUIRED_CAPABILITY ) ) {
				echo self::render_template( 'yd-settings/page-settings.php' );
			} else {
		...
		public function markup_fields( $field ) {
			...
			echo self::render_template( 'yd-settings/page-settings-fields.php', array( 'settings' => $this->settings, 'field' => $field ), 'always' );
		}
		...
		public function validate_settings( $new_settings ) {
			...
			if ( ! is_string( $new_settings['db-version'] ) ) {
				$new_settings['db-version'] = YD_Plugin_Skeleton::VERSION;
			}
		...
		public static function add_user_fields( $user ) {
			echo self::render_template( 'yd-settings/user-fields.php', array( 'user' => $user ) );
		}
		...
		public static function save_user_fields( $user_id ) {
			...
			update_user_meta( $user_id, 'yd_user-example-field1', $user_fields[ 'yd_user-example-field1' ] );
			update_user_meta( $user_id, 'yd_user-example-field2', $user_fields[ 'yd_user-example-field2' ] );
		}
		...
		public static function validate_user_fields( $user_id, $user_fields ) {
			if ( $user_fields[ 'yd_user-example-field1' ] == false ) {
				$user_fields[ 'yd_user-example-field1' ] = true;
				add_notice( 'Example Field 1 should be true', 'error' );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				$current_field2 = get_user_meta( $user_id, 'yd_user-example-field2', true );

				if ( $current_field2 != $user_fields[ 'yd_user-example-field2' ] ) {
					$user_fields[ 'yd_user-example-field2' ] = $current_field2;
