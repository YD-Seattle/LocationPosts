...
require_once( dirname( dirname( __DIR__ ) ) . '/classes/yd-settings.php' );

/**
 * Unit tests for the YD_Settings class
 *
 * Uses the SimpleTest For WordPress plugin
 *
 * @link http://wordpress.org/extend/plugins/simpletest-for-wordpress/
 */
if ( ! class_exists( 'UnitTestYD_Settings' ) ) {
	class UnitTestYD_Settings extends UnitTestCase {
		public function __construct() {
			$this->YD_Settings = YD_Settings::get_instance();
		}
		...
		public function test_validate_settings() {
			...
			$clean_settings = $this->YD_Settings->validate_settings( $valid_settings );
			...
			$clean_settings = $this->YD_Settings->validate_settings( $invalid_settings );
		...
		public function test_magic_set() {
			...
			$this->YD_Settings->init();
			$this->YD_Settings->settings = array( 'db-version' => array() );
			$this->assertEqual( $this->YD_Settings->settings['db-version'], YD_Plugin_Skeleton::VERSION );

			// Test that values gets written to database
			$this->YD_Settings->settings = array( 'db-version' => '5' );
			$this->YD_Settings->init();
			$this->assertEqual( $this->YD_Settings->settings['db-version'], '5' );
			$this->YD_Settings->settings = array( 'db-version' => YD_Plugin_Skeleton::VERSION );
			...
			$this->YD_Settings->settings['db-version'] = YD_Plugin_Skeleton::VERSION;
		}
	} // end UnitTestYD_Settings
}
